...
class PyGame:
    def __init__(self):
        ...
        # Offset for vertical centering in fullscreen
        self.offset_y = 0
    ...
    def toggle_fullscreen(self):
        ...
        if self.fullscreen:
            ...
            # Scale based on width to fit the monitor, preserving aspect ratio
            self.fullscreen_scale_factor = fullscreen_width / self.base_width
            self.WIDTH = int(self.base_width * self.fullscreen_scale_factor)
            self.HEIGHT = int(self.base_height * self.fullscreen_scale_factor)
            # Calculate vertical offset for centering (black bars above and below)
            self.offset_y = (fullscreen_height - self.HEIGHT) // 2
            self.monitor_height = fullscreen_height
        else:
    ...
    def draw_background(self, win):
        ...
        for i in range(-1, building_tiles):
            ...
            for b in self.buildings:
                ...
                win.blit(b_scaled, (pos_x, int(60 * self.effective_scale) + self.offset_y))
        ...
        for i in range(-1, wall_tiles):
            ...
            if -self.wall_width <= pos_x <= self.WIDTH:
                ...
                win.blit(wall, (pos_x, int(380 * self.effective_scale) + self.offset_y))
        ...
        for i in range(footpath_tiles):
            ...
            if pos_x > self.WIDTH or pos_x < -self.footpath_scaled_width:
                continue
            win.blit(footpath, (pos_x, int(560 * self.effective_scale) + self.offset_y))
        ...
        for i in range(-1, road_tiles):
            ...
            if -self.road_width <= pos_x <= self.WIDTH:
                ...
                win.blit(road, (pos_x, int(620 * self.effective_scale) + self.offset_y))
    ...
    def draw_enemies(self, win):
        ...
        for enemy in self.enemies[:]:
            ...
            if enemy.ready_to_remove:
                ...
                continue
            enemy.draw(win, scroll_offset, self.offset_y)
    ...
    def check_combo_hits(self, win):
        if self.is_comboing:
            ...
            for enemy in self.enemies[:]:
                ...
                pygame.draw.circle(win, GREEN, (int(enemy_screen_center), self.y + int(200 * self.effective_scale) + self.offset_y), int(5 * self.effective_scale))
    ...
    def draw_wave_info(self, win):
        ...
        win.blit(wave_text, (int(10 * self.effective_scale), int(10 * self.effective_scale) + self.offset_y))
        if self.current_wave >= self.MAX_WAVES and not self.wave_in_progress and not self.enemies:
            ...
            win.blit(win_text, (self.WIDTH // 2 - win_text.get_width() // 2, self.HEIGHT // 4 + self.offset_y))
    ...
    def main(self):
        ...
        while run:
            ...
            # Clear the entire screen (including areas for black bars)
            win.fill(BLACK)
            ...
            if self.is_dying:
                game_surface.blit(char_death_left if self.facing_left else char_death_right, (self.screen_x, self.y))
            elif self.is_hit:
                game_surface.blit(char_hit_left if self.facing_left else char_hit_right, (self.screen_x, self.y))
            elif self.is_comboing:
                game_surface.blit(char_combo_left if self.facing_left else char_combo_right, (self.screen_x, self.y))
            elif not keys[pygame.K_LEFT] and not keys[pygame.K_RIGHT]:
                game_surface.blit(char_idle_left if self.facing_left else char_idle_right, (self.screen_x, self.y))
            elif keys[pygame.K_RSHIFT]:
                game_surface.blit(char_run_right if self.walk_right else char_run_left, (self.screen_x, self.y))
            else:
                game_surface.blit(char_walk_left if self.walk_left else char_walk_right, (self.screen_x, self.y))

            # Blit the game surface onto the window, centered vertically
            win.blit(game_surface, (0, self.offset_y))
